                    ...
                    <template #item="{ element }">
                        <el-row class="item-box">
                            ...
                            <el-col :sapn="24" class="text-center">
                                <img v-if="element.image" :src="element.image" width="100" onerror="this.src='/sistema/images/forum/default.png';" />
                                <img v-else src="/sistema/images/forum/default.png" width="100" />
                            </el-col>
                        </el-row>
                    </template>
                </draggable>
    ...
    export default {
        ...
        setup() {
            ...
            const requestForum = async() => {
                ...
                // request error
                if(!response.success) {
                    ElMessage.error('No se ha logrado obtener el listado del foro, intente nuevamente por favor')
                    loading.close()
                    return
                }
            ...
            const saveOrder = async() => {
                ...
                let response = await request.send({
                    method: 'POST',
                    url: import.meta.env.VITE_API_SYSTEM_URL + '/forum/order',
                    headers: {
                        'content-type': 'application/json',
                        'session': appStore.session
                    }, 
