                ...
                <el-row>
                    <el-col :span="24">
                        <el-breadcrumb separator="/">
                            ...
                            <el-breadcrumb-item>MEMBRESIA</el-breadcrumb-item>
                        </el-breadcrumb>
                    </el-col>
                </el-row>
            </el-header>
                ...
                <el-row>
                    <el-col :span="24">
                        ...
                        >
                            ...
                            >
                                <template #default="scope" v-if="item.key == 'active'">
                                ...
                                <template #default="scope" v-if="item.key == 'actions'">
                                    ...
                                    <div class="table-icon cursor-pointer">
                                        <box-icon name='block' color='#760000' @click="status(scope.row.id, scope.row.name)"></box-icon>
                                    </div>
                                </template>
                            </el-table-column>
                        </el-table>
    ...
    export default {
        setup() {
            ...
            const status = (id, name) => {
                ElMessageBox.confirm(`¿Quieres cambiar el estatus de la membresía "${name}" con id ${id}?`)
                .then(() => {
                    requestStatus(id)
                })
                .catch(() => {
            ...
            const requestMembreship = async() => {
                ...
                let response = await request.send({
                    method: 'GET',
                    url: import.meta.env.VITE_API_SYSTEM_URL + '/membership',
                    headers: {
                ...
                // request error
                if(!response.success) {
                    ElMessage.error('No se ha logrado obtener el listado de membresías, intente nuevamente por favor')
                    loading.close()
                    return
                }
            ...
            const requestStatus = async(id) => {
                ...
                let response = await request.send({
                    method: 'DELETE',
                    url: `${import.meta.env.VITE_API_SYSTEM_URL}/membership/${id}`,
                    headers: {
                        'content-type': 'application/json',
                        'session': appStore.session
                    }
                ...
                // request error
                if(!response.success) {
                    ElMessage.error('No se ha logrado actualizar el estatus de esta membresía, intenta nuevamente por favor')
                    loading.close()
                    return
                }
                response = response.response
