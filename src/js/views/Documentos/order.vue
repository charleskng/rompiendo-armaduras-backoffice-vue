                    ...
                    <template #item="{ element }">
                        <el-row class="item-box">
                            ...
                            <el-col :sapn="24" class="text-center">
                                <span style="white-space: pre;">{{ element.desc }}</span>
                            </el-col>
                        </el-row>
                    </template>
                </draggable>
    ...
    export default {
        ...
        setup() {
            ...
            const requestForum = async() => {
                ...
                // request error
                if(!response.success) {
                    ElMessage.error('No se ha logrado obtener el listado de documentos, intente nuevamente por favor')
                    loading.close()
                    return
                }
            ...
            const saveOrder = async() => {
                ...
                let response = await request.send({
                    method: 'POST',
                    url: import.meta.env.VITE_API_SYSTEM_URL + '/docs/order',
                    headers: {
                        'content-type': 'application/json',
                        'session': appStore.session
                    }, 
