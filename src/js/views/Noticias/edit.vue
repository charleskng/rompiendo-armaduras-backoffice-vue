...
<script>
    ...
    export default {
        setup() {
            ...
            const imageDefault = `${import.meta.env.VITE_STORAGE}/images/news/default.png`
            let image = ref(imageDefault)
            let image_files = reactive([])
            let upload_endpoint = ref(`${import.meta.env.VITE_API_SYSTEM_URL}/news/image/${route.params.id}`)
            ...
            const requestNews = async() => {
                ...
                let response = await request.send({
                    method: 'GET',
                    url: `${import.meta.env.VITE_API_SYSTEM_URL}/news/${route.params.id}`,
                    headers: {
            ...
            const requestEdit = async() => {
                ...
                if(error === null) {
                    ...
                    // request error
                    if(!response.success) {
                        ElMessage.error('No se ha logrado actualizar la noticia, intente nuevamente por favor')
                        loading.close()
                        return
                    }
                    response = response.response
                    ...
                    ElMessage.success('Se ha actualizado la noticia exitosamente')
