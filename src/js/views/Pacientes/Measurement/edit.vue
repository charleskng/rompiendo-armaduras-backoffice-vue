                ...
                <el-row>
                    <el-col :span="24">
                        <el-breadcrumb separator="/">
                            ...
                            <el-breadcrumb-item>{{ measurement_id }}</el-breadcrumb-item>
                        </el-breadcrumb>
                    </el-col>
                </el-row>
            </el-header>
        ...
        setup() {
            ...
            const requestMeasurements = async() => {
                ...
                let response = await request.send({
                    method: 'GET',
                    url: `${import.meta.env.VITE_API_CONSULTANT_URL}/${route.params.id}/measurement/${route.params.measurement_id}`,
                    headers: {
                ...
                // request error
                if(!response.success) {
                    ElMessage.error('No se ha logrado obtener las mediciones del paciente, intente nuevamente por favor')
                    loading.close()
                    return
                }
                ...
                form.weight_kg = response.data.measurement.weight_kg
                form.fat_percentage = response.data.measurement.fat_percentage
                form.fat_kg = response.data.measurement.fat_kg
                form.muscle_kg = response.data.measurement.muscle_kg
                form.bone_mass = response.data.measurement.bone_mass
                form.bmi = response.data.measurement.bmi
                form.metabolic_age = response.data.measurement.metabolic_age
                form.water_percentage = response.data.measurement.water_percentage
                form.visceral_fat = response.data.measurement.visceral_fat
                form.waist_circumference = response.data.measurement.waist_circumference
                form.abdomen_circumference = response.data.measurement.abdomen_circumference
                form.hip_circumference = response.data.measurement.hip_circumference
                form.date = new Date(response.data.measurement.creation)
                form.time = new Date(response.data.measurement.creation)
