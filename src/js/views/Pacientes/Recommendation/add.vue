            ...
            const requestAdd = async() => {
                ...
                if(error === null) {
                    ...
                    let response = await request.send({
                        method: 'POST',
                        url: `${import.meta.env.VITE_API_CONSULTANT_URL}/${route.params.id}/recommendation/add`,
                        headers: {
                    ...
                    // request error
                    if(!response.success) {
                        ElMessage.error('No se ha logrado guardar esta recomendación, intente nuevamente por favor')
                        loading.close()
                        return
