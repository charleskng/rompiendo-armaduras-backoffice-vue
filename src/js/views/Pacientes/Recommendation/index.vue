                ...
                <el-row>
                    <el-col :span="24">
                        <el-breadcrumb separator="/">
                            ...
                            <el-breadcrumb-item>RECOMENDACIONES</el-breadcrumb-item>
                        </el-breadcrumb>
                    </el-col>
                </el-row>
            </el-header>
        ...
        setup() {
            ...
            const requestRecommendations = async() => {
                ...
                let response = await request.send({
                    method: 'GET',
                    url: `${import.meta.env.VITE_API_CONSULTANT_URL}/${route.params.id}/recommendation`,
                    headers: {
            ...
            const deleteItem = (recommendation_id) => {
                ElMessageBox.confirm(`¿Quieres eliminar la recomendación con id ${recommendation_id}?`)
                .then(() => {
                    requestDelete(recommendation_id)
                })
                .catch(() => {
                    log.debug('Delete recommendation cancelled')
                })
            }

            const requestDelete = async(recommendation_id) => {
                // show loading
                const loading = ElLoading.service({
                    lock: true,
                    text: 'Actualizando información',
                ...
                let response = await request.send({
                    method: 'DELETE',
                    url: `${import.meta.env.VITE_API_CONSULTANT_URL}/${route.params.id}/recommendation/${recommendation_id}`,
                    headers: {
                        'content-type': 'application/json',
                        'session': appStore.session
                    }
                ...
                // request error
                if(!response.success) {
                    ElMessage.error('No se han logrado eliminar esta recomendación, intenta nuevamente por favor')
                    loading.close()
                    return
                }
                response = response.response
